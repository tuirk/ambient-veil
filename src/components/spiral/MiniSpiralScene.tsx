...
import { MiniSpiralLine } from "./MiniSpiralLine";
import { MiniMonthMarkers } from "./MiniMonthMarkers";
import { EventVisualizations } from "./EventVisualizations";

interface MiniSpiralSceneProps {
  events: TimeEvent[];
  config: SpiralConfig;
  onEventClick: (year: number, month: number, x: number, y: number) => void;
}

export const MiniSpiralScene: React.FC<MiniSpiralSceneProps> = ({ 
  events, 
  config, 
  onEventClick 
}) => {
