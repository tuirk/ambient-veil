...
/**
 * Visual representation of future events, rendered as floating geometry
 */
export const FutureEvent: React.FC<FutureEventProps> = ({ event, onEventClick }) => {
  ...
  if (event.intensity > 7) {
    geometry = <octahedronGeometry args={[0.2 + event.intensity * 0.03, 0]} />;
  } else if (event.intensity > 4) {
    geometry = <tetrahedronGeometry args={[0.2 + event.intensity * 0.03, 0]} />;
  } else {
    geometry = <dodecahedronGeometry args={[0.15 + event.intensity * 0.02, 0]} />;
  }
